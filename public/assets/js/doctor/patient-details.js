...
if (patientDetailsUser) {
    ...
    let currentAppointmentId = appointmentIdParam
    ...
    async function loadPatient() {
        ...
        try {
            const query = currentAppointmentId
                ? `?appointment_id=${currentAppointmentId}`
                : "";
            const response = await fetch(
                `${API_URL}/doctor/patients/${patientId}${query}`,
                {
                    headers: authHeaders(),
                },
            );
    ...
    function renderAppointment(appointment) {
        if (!appointment) {
            document.getElementById("appointmentInfo").innerHTML =
                `<li class="text-muted">لا يوجد موعد مرتبط</li>`;
    ...
    function makeHistoryTable(headers, rows) {
        if (!rows.length)
            return `<div class="text-muted">لا توجد بيانات سابقة</div>`;
        return `
    ...
    function renderMedicalHistory(
      ...
      `,
        ...
        document.getElementById("prescriptionsTab").innerHTML =
            makeHistoryTable(
    ...
    function renderDraft() {
        document.getElementById("prescriptionDraft").innerHTML =
            prescriptionDraft
                .map(
                    (item, index) => `
      <tr>
    ...
    document
        .getElementById("saveDiagnosisBtn")
        .addEventListener("click", async () => {
            if (!currentAppointmentId) {
                showAlert(
                    "patientMessage",
                    "لا يوجد موعد لحفظ التشخيص عليه",
                    "danger",
                );
                return;
            }
            try {
                const response = await fetch(
                    `${API_URL}/doctor/appointments/${currentAppointmentId}`,
                    {
                        method: "PATCH",
                        headers: authHeaders({
    ...
    document
        .getElementById("completeAppointmentBtn")
        .addEventListener("click", async () => {
            if (!currentAppointmentId) {
    ...
    async function sendMedicalFileRequest(
        fileType,
        requestNameElementId,
        notesElementId,
        successMessage,
    ) {
        try {
            const response = await fetch(`${API_URL}/doctor/medical-files`, {
                ...
                body: JSON.stringify({
                    ...
                    file_type: fileType,
                    request_name:
                        document.getElementById(requestNameElementId).value,
    ...
    document
        .getElementById("sendRadiologyBtn")
    ...
    document
        ...
        .addEventListener("click", () => {
            ...
            if (!medication || !dosage || !instruction) {
                showAlert(
                    "patientMessage",
                    "أكملي بيانات الدواء قبل الإضافة",
                    "danger",
                );
                return;
            }
    ...
    document
        .getElementById("prescriptionDraft")
        .addEventListener("click", (event) => {
            const index = event.target.closest("[data-remove-prescription]")
                ?.dataset.removePrescription;
    ...
    document
        .getElementById("savePrescriptionsBtn")
        .addEventListener("click", async () => {
