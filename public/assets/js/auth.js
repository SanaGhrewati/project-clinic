...
function normalizeUser(user) {
    ...
    const doctorType =
        user?.doctor_type || user?.doctorType || user?.type || "";
...
function dashboardRouteFor(user) {
    ...
    if (normalized.role === "Lab" || normalized.role === "Radiology")
        return APP_ROUTES.staffDashboard;
...
async function login(email, password) {
    ...
    if (!response.ok) {
        throw new Error(
            data.message || "فشل تسجيل الدخول، يرجى التأكد من البيانات.",
        );
    }
